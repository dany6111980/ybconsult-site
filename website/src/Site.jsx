...
/* simple, non-breaking language switcher */
const LangSwitch = () => {
  const langs = [
    { code: "en", label: "EN" },
    { code: "fr", label: "FR" },
  ...
  return (
    <div className="flex items-center gap-2 text-xs">
      {langs.map((l) => (
        <a
          key={l.code}
          href={`?lang=${l.code}${window.location.hash || ""}`}
...
export default function Site() {
  ...
  /* expose a minimal ask button handler that works even if the script isn't mounted yet */
  const openAskYB = () => {
    if (window.AskYB && typeof window.AskYB.open === "function") {
      window.AskYB.open();
    } else {
      alert("The assistant will be available shortly. Please use the form or email meanwhile.");
    }
  };
  ...
  return (
    <div className="min-h-screen text-slate-900 bg-slate-50 bg-grid">
      ...
      <header className="sticky top-0 z-40 backdrop-blur bg-white/75 border-b border-slate-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-6 text-sm">
            ...
            {/* non-breaking language switcher */}
            <LangSwitch />
          </nav>
          <button
      ...
      <section id="contact" className="py-16 md:py-20 bg-slate-900 text-white relative overflow-hidden">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
            ...
            <div className="lg:pl-2">
              ...
              <div className="mt-6 rounded-2xl border border-slate-200 bg-white p-6">
                ...
                {/* Registered office (per Yves) */}
                <p className="mt-4 text-xs text-slate-500">
                  <span className="font-semibold">Registered office:</span> YBConsulting, 2 bis rue Saint Léger, 1205 Genève, Switzerland.
                </p>
              </div>
            </div>
          </div>
        ...
        {/* Floating “Ask YB” button (replaces chat widget) */}
        <button
          onClick={openAskYB}
          aria-label="Ask YB"
          className="fixed bottom-6 right-6 inline-flex items-center gap-2 rounded-full px-4 py-3 shadow-lg bg-indigo-600 hover:bg-indigo-700 text-white"
        >
